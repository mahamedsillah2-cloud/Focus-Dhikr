 ...
 * every single time, with the authority who gave it. A report whose authenticity
 * could not be stated plainly is simply not here.
 *
 * `tools/verify_citations.py` checks each reference resolves on sunnah.com and
 * that the Arabic below appears in the fetched text.
 */
object HadithLibrary {

    val all: List<HadithCitation> = listOf(
        ...
        HadithCitation(
            ...
            partial = true,
            themes = setOf(Theme.VALUE_OF_TIME),
        ),
        ...
        HadithCitation(
            ...
            partial = true,
            themes = setOf(Theme.SELF_DISCIPLINE, Theme.RESTRAINING_DESIRE),
        ),
    )

    fun byTheme(theme: Theme): List<HadithCitation> = all.filter { theme in it.themes }

    fun pick(theme: Theme?, seed: Int): HadithCitation {
